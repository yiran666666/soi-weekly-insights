"""Generate P0 Investigate report: SOI Down + Spend Up accounts — styled HTML."""
import pandas as pd
import matplotlib
matplotlib.use("Agg")
...
def html_impact_table(df_apps, df_total):
    ...
    return f"""<p class="note">Which apps are dragging SOI down despite higher spend? <strong>SOI Impact</strong> = change in app's Moloco install share of total installs. Negative = unattributed installs growing faster than Moloco. <span class="highlight-label">Highlighted rows</span> = top spend apps (&ge;10% share).</p>
    <table>
        <thead><tr>
...
def main():
    ...
    <title>P0 Investigate: Spend Up SOI Down</title>{CSS}</head><body>"""]

    html.append("<h1>P0 Investigate: Spend Up + SOI Down — Deep Dive</h1>")
    html.append('<p class="meta"><strong>Accounts:</strong> TIKTOK_US, Mattel163_China, Learnings, ZerooGravity, MICROFUN<br>')
    html.append("<strong>Period:</strong> Benchmark (Dec 25-31, 2025) → Mar 1-13, 2026<br>")
    html.append("<strong>Criteria:</strong> Avg daily spend UP but SOI DOWN in March vs Benchmark<br>")
    html.append("<strong>Key question:</strong> Is the unattributed install surge from organic growth, other DSPs scaling up, or market-wide expansion?<br>")
    ...
    path = os.path.join(OUTPUT_DIR, "p0_investigate_deep_dive.html")
