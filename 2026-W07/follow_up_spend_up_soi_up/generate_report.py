"""Generate P0 Protect accounts report as a styled HTML file."""
import pandas as pd
import matplotlib
matplotlib.use("Agg")
...
DATA_PATH = "/Users/yiran.shi/Desktop/2026_playground_moloco_center/claude-bq-agent/tmp/data/20260317_155119_dec6.csv"
...
ACCOUNT_SHORT = {
    "Wedobest_Screw Sort Puzzle_Madhouse_1226_01(VpSvKE3W1NWor3jZ)": "Wedobest (Screw Sort Puzzle)",
}
CJK_APP_NAMES = {
    "セルサバイバー - ディフェンスゲーム": "Cell Survivor (JP)",
    "RO仙境傳説：世界之旅": "RO World Journey (TW)",
}
...
def html_impact_table(df_apps, df_total):
    ...
    return f"""<p class="note">Which apps drive the overall SOI change? <strong>SOI Impact</strong> = change in app's Moloco install share of total installs. <span class="highlight-label">Highlighted rows</span> = top spend apps (&ge;10% share).</p>
    <table>
        <thead><tr>
...
def main():
    ...
    <title>P0 Protect Deep Dive</title>{CSS}</head><body>"""]

    html.append("<h1>P0 Protect: SOI Up + Spend Up — Deep Dive</h1>")
    html.append('<p class="meta"><strong>Accounts:</strong> CENTURY_GAMES, Wedobest (Screw Sort Puzzle), Luckymoney, AVIAGAMES, 37GAMES<br>')
    html.append("<strong>Period:</strong> Benchmark (Dec 25-31, 2025) → Mar 1-13, 2026<br>")
    html.append("<strong>Criteria:</strong> Both avg daily spend AND SOI increased in March vs Benchmark<br>")
    ...
    for acct in account_order:
        ...
        if not acct_apps.empty and not acct_total.empty:
            html.append("<h3>SOI Impact by App (Benchmark → Mar)</h3>")
    ...
    html.append('<p class="meta"><em>Generated 2026-03-17. SOI = Moloco Installs / (Moloco + Non-Moloco Installs). DRR = Avg Daily Spend.</em></p>')
    html.append("</body></html>")

    path = os.path.join(OUTPUT_DIR, "p0_protect_deep_dive.html")
