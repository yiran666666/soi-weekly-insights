"""Generate P0 Accelerate report: SOI Up + Spend Down accounts — styled HTML."""
import pandas as pd
import matplotlib
matplotlib.use("Agg")
...
ACCOUNT_SHORT = {
    "Tigo_Tigo_Madhouse_0108_01": "Tigo (Madhouse)",
}
CJK_APP_NAMES = {}
...
def html_impact_table(df_apps, df_total):
    ...
    return f"""<p class="note">Which apps are driving the SOI gain despite lower spend? <strong>SOI Impact</strong> = change in app's Moloco install share of total installs. Positive = Moloco gaining share as unattributed installs decline. <span class="highlight-label">Highlighted rows</span> = top spend apps (&ge;10% share).</p>
    <table>
        <thead><tr>
...
def main():
    ...
    <title>P0 Accelerate: Spend Down SOI Up</title>{CSS}</head><body>"""]

    html.append("<h1>P0 Accelerate: SOI Up + Spend Down — Deep Dive</h1>")
    ...
    html.append("<strong>Criteria:</strong> Avg daily spend DOWN but SOI UP in March vs Benchmark<br>")
    html.append("<strong>Key question:</strong> Why is spend declining? Can we restore or grow budget to compound the SOI gains?<br>")
    ...
    path = os.path.join(OUTPUT_DIR, "p0_accelerate_deep_dive.html")
